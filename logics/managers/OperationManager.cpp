	...
	for (int i = 0; i < cards.count(); ++i) {
		if (cards[i]->number() == cardNumber) {
			if (cards[i]->isBlocked())
				throw BlockedCardException(cardNumber);
...
void OperationManager::takeCredit(const QString& name, uint amount, uint period, const QDateTime& start,
								  const QDateTime& end) {
...
const QList<RegularPayment*> OperationManager::getAllCustomerPayments() {
	return _customerDataManager.regularPayments();
}
